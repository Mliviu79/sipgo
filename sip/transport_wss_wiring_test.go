...
// TestCreateConnectionPoolsUnderEveryAddressTheFarEndMayUse is the regression.
// After a real dial, the socket must be findable under every form the registrar
// might address it by -- and, critically, must NOT be findable only under our
// own local address, which is what the transposed argument order produced.
func TestCreateConnectionPoolsUnderEveryAddressTheFarEndMayUse(t *testing.T) {
	...
	// Hostname set alongside IP is the production shape: we dial the registrar by
	// name so TLS can verify its certificate, but it addresses us back by IP.
	...
	// Every form the far end may drive a lookup from. The real peer is reachable
	// as ct25.iset.ro:8089, ct25.iset.ro, 194.102.34.49 and 194.102.34.49:8089;
	// all four must resolve to the one open socket.
	for _, key := range []string{
		net.JoinHostPort(ip.String(), portStr),
		ip.String(),
		net.JoinHostPort(tlsServerName, portStr),
		tlsServerName,
	} {
		if got := tran.pool.Get(key); got == nil {
			t.Errorf("no pooled connection under %q: a lookup the far end drives by this address "+
				"finds nothing, so the registrar cannot route a request back down the socket we "+
				"opened. CreateConnection must key the pool on raddr and pass wsConnectionAliases(raddr).", key)
		}
	}
}
