...
func TestTransportParseFailureLog(t *testing.T) {
	...
	tests := []struct {
		...
		{"TCP", func(t *testing.T, log *slog.Logger) {
			...
			par := NewParser()
			tr.init(par)
			// Logging the failure may not swallow it: the caller closes the
			// connection on some parse errors.
			err := tr.parseStream(par.NewSIPStream(), malformedRegister, src, handler)
			require.Error(t, err)
		}},
